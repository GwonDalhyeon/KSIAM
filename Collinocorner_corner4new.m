...
for j= 2:Ny-1
    Phi_1(:,j,1) = 2* Phi_1(:,j,2) - Phi_1(:,j,1) + (dt)^2/(dx)^2 * (Psi_1(:,j-1,2) - 2*Psi_1(:,j,2) + Psi_1(:,j+1,2)) ;
    Phi_3(:,j,1) = 2* Phi_3(:,j,2) - Phi_3(:,j,1) + (dt)^2/(dx)^2 * (Psi_3(:,j-1,2) - 2*Psi_3(:,j,2) + Psi_3(:,j+1,2)) ;
end
%Phi(:,:,1) - n+1 time
%Phi(:,:,2) - n time
